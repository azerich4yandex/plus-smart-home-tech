...
public class ScenarioAddedEventHandler extends BaseHubEventHandler {
    ...
    public ScenarioAddedEventHandler(KafkaTemplate<String, SpecificRecordBase> kafkaTemplate) {
        super(kafkaTemplate);
    }

    @Override
    protected HubEvent mapHubEventProtoToModel(HubEventProto eventProto) {
        HubEvent event = HubEventProtoMapper.toScenarioAddedEvent(eventProto);
